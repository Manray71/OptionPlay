...
def detect_short_sma_convergence(df: pd.DataFrame, lookback: int = 10) -> pd.DataFrame:
    ...
    df["convergence_rate"] = (
        (df["short_spread_prev"] - df["short_sma_spread_pct"]) / df["short_spread_prev"] * 100
    )
    ...
    df["approaching_sma120"] = (
        df["dist_to_sma120_pct"].abs() < df["dist_to_sma120_pct"].shift(lookback).abs()
    )
...
def simulate_weekly_timeframe(df: pd.DataFrame, lookback: int = 20) -> pd.DataFrame:
    ...
    df["weekly_short_spread"] = (
        (df[weekly_short].max(axis=1) - df[weekly_short].min(axis=1)) / df["close"] * 100
    )
...
def run_analysis():
    ...
    configs = [
        ...
        {
            "name": "Kurze SMAs konvergieren (unter 120)",
            "spread": 2.0,
            "below120": True,
            "above120": False,
            "above250": False,
        ...
        {
            "name": "Kurze SMAs konvergieren (über 120)",
            "spread": 2.0,
            "below120": False,
            "above120": True,
            "above250": False,
        ...
        {
            "name": "Unter 120, ÜBER 250 (bullish LT)",
            "spread": 2.0,
            "below120": True,
            "above120": False,
            "above250": True,
        ...
        {
            "name": "Unter 120, UNTER 250 (bearish LT)",
            "spread": 2.0,
            "below120": True,
            "above120": False,
            "above250": False,
        ...
        {
            "name": "Über 120, ÜBER 250 (strong bull)",
            "spread": 2.0,
            "below120": False,
            "above120": True,
            "above250": True,
        ...
        {
            "name": "Über 120, UNTER 250 (bear rally)",
            "spread": 2.0,
            "below120": False,
            "above120": True,
            "above250": False,
        ...
        {
            "name": "Unter 120 + Weekly + über 250",
            "spread": 2.0,
            "below120": True,
            "above120": False,
            "above250": True,
        ...
        # Lockerer Spread
        {
            "name": "Spread 3%, unter 120, über 250",
            "spread": 3.0,
            "below120": True,
            "above120": False,
            "above250": True,
        ...
        if all_results:
            ...
            for days in [5, 10, 20, 30]:
                ...
                if len(subset) >= 10:
                    ...
                    print(
                        f"  {days:2}d Hold: n={len(subset):4}, WR={wr:5.1f}%, Avg={avg:+6.2f}%, Sharpe={sharpe:.3f}"
                    )
        else:
        ...
        if len(subset_20d) > 0:
            ...
            for bucket in ["<10%", "10-20%", "20-30%", "30-50%", ">50%"]:
                ...
                if len(bucket_data) >= 5:
                    ...
                    print(
                        f"  Konvergenz {bucket}: n={len(bucket_data):3}, WR={wr:5.1f}%, Avg={avg:+6.2f}%"
                    )
        ...
        if len(subset_20d) > 0:
            ...
            for bucket in ["Über 120", "0-2%", "2-5%", "5-10%", ">10%"]:
                ...
                if len(bucket_data) >= 5:
                    ...
                    avg = bucket_data.mean()
                    print(
                        f"  Distanz {bucket}: n={len(bucket_data):3}, WR={wr:5.1f}%, Avg={avg:+6.2f}%"
                    )

        # Top Symbole
        print(f"\n--- Top 15 Symbole (20d Hold) ---")
        symbol_stats = (
            subset_20d.groupby("symbol")
            .agg({"return": ["count", "mean", lambda x: (x > 0).mean() * 100]})
            .round(2)
        )
    ...
    for setup_name, results_df in results_by_setup.items():
        for days in [10, 20]:
            ...
            if len(subset) >= 5:
                ...
                sharpe = avg / subset.std() if subset.std() > 0 else 0
                print(
                    f"{setup_name:<30} {days:<6} {len(subset):<6} {wr:<8.1f} {avg:<+10.2f} {sharpe:<8.3f}"
                )
