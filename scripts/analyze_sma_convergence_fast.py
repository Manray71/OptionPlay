...
def analyze_convergence_strategy():
    ...
    for params in param_grid:
        ...
        for symbol in df["symbol"].unique():
            ...
            symbol_df["sma_spread"] = (
                (symbol_df[sma_cols].max(axis=1) - symbol_df[sma_cols].min(axis=1))
                / symbol_df["close"]
                * 100
            )
        ...
        if signals_list:
            ...
            print(
                f"  Trades: {len(valid)}, Win Rate: {win_rate:.1f}%, Avg: {avg_return:+.2f}%, Sharpe: {sharpe:.3f}"
            )
...
def analyze_by_vix_regime():
    ...
    # VIX-Daten laden
    vix_df = pd.read_sql_query("SELECT date, value as vix FROM vix_data ORDER BY date", conn)
