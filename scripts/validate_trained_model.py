...
def analyze_symbol_stability(model: Dict) -> Dict:
    ...
    return {
        ...
        'stability_rate': len(stable_symbols) / len(all_symbols) * 100
...
def print_validation_report(model: Dict):
    ...
    print(f"  Win Rate:         {agg['train_wr']:>14.2f}%  {agg['test_wr']:>14.2f}%  {agg['overall_degradation']:>+11.2f}%")
    print()
    ...
    for regime in ['low', 'normal', 'elevated', 'high']:
        ...
        for strategy, data in perf[regime].items():
            ...
            print(f"    {strategy:15s} | Train: {train_wr:5.1f}% ({train_n:5d}) | "
    ...
    if stable:
        ...
        for s in stable[:10]:
            print(f"    {s['symbol']:6s} | WR: {s['win_rate']:5.1f}% | "
        ...
        for strategy, buckets in iv_analysis.items():
            ...
            for bucket in ['0-25', '25-50', '50-75', '75-100', '100-125']:
                if bucket in buckets:
                    ...
                    values.append("   -")
            print(f"  {strategy:15s} | {values[0]:>10s} | {values[1]:>10s} | "
    ...
    if agg['avg_degradation'] < 0:
        strengths.append(f"Negative Durchschnittsdegradation ({agg['avg_degradation']:+.2f}%) - KEIN Overfitting!")
    elif agg['avg_degradation'] < OVERFIT_NONE:
        strengths.append(f"Minimale Degradation ({agg['avg_degradation']:+.2f}%) - Kein signifikantes Overfitting")
