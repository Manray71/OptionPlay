...
def detect_upward_convergence(df: pd.DataFrame, lookback: int = 10) -> pd.DataFrame:
    ...
    df["convergence_speed"] = (
        (df["spread_prev"].abs() - df["spread_pct"].abs()) / df["spread_prev"].abs() * 100
    )
...
def find_convergence_signals(df: pd.DataFrame, require_weekly: bool = False) -> pd.DataFrame:
    """
...
def analyze_breakouts(
    df: pd.DataFrame, signals: pd.DataFrame, holding_days: int = 20
) -> pd.DataFrame:
