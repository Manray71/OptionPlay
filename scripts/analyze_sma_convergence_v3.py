...
def simulate_weekly_timeframe(df: pd.DataFrame, max_spread_pct: float = 5.0) -> pd.DataFrame:
    ...
    df["weekly_convergence"] = (df["wsma_spread"] < max_spread_pct) & df["weekly_aligned"]

    return df


def find_signals(
    df: pd.DataFrame, require_weekly: bool = False, min_rsi: float = 45, max_rsi: float = 65
) -> pd.DataFrame:
        ...
        if daily_results:
            ...
            for days in [10, 20, 30]:
                ...
                if len(subset) > 20:
                    ...
                    print(
                        f"Daily Only   ({days}d): n={len(subset):4}, WR={wr:5.1f}%, Avg={avg:+6.2f}%, Sharpe={sharpe:.3f}"
                    )
        ...
        if combined_results:
            ...
            for days in [10, 20, 30]:
                ...
                if len(subset) > 10:
                    ...
                    print(
                        f"Daily+Weekly ({days}d): n={len(subset):4}, WR={wr:5.1f}%, Avg={avg:+6.2f}%, Sharpe={sharpe:.3f}"
                    )
