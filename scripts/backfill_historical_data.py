#!/usr/bin/env python3
"""
OptionPlay - Historical Data Backfill Script
=============================================

Erweitert historische Daten rückwärts für Symbole mit kurzer Historie.
Holt ältere Daten ab, die vor dem aktuellen Start-Datum liegen.

Usage:
    # Status anzeigen
    python scripts/backfill_historical_data.py --status

    # Dryrun
    python scripts/backfill_historical_data.py --dryrun

    # Backfill ausführen (Standard: 3 Jahre zurück)
    python scripts/backfill_historical_data.py --backfill

    # Mit spezifischer Zieltiefe
    python scripts/backfill_historical_data.py --backfill --target-years 5
"""
...
class BackfillCollector:
    """Sammelt historische Daten rückwärts"""
    ...
    async def backfill_symbol(
        ...
        try:
            # Hole volle Historie (1500 Tage = ca. 6 Jahre)
            # Die API berechnet from_date von heute aus, also müssen wir
            # genug Tage anfordern um bis zum target_start zu kommen
            today = date.today()
            days_from_today = (today - info.target_start).days + 100  # Puffer

            # Maximal 1500 Tage (API Limit)
            days_to_fetch = min(days_from_today, 1500)
...
async def run_backfill(results: List[BackfillInfo], config: BackfillConfig, dryrun: bool = False):
    ...
    api_key = get_api_key()
    tracker = TradeTracker()
    collector = BackfillCollector(api_key, config)
    ...
    print(f"\nProcessing {len(to_process)} symbols for backfill...")
...
async def main():
    parser = argparse.ArgumentParser(
        description="Backfill historical data for symbols with short history",
    ...
    parser.add_argument(
        "--delay", type=float, default=0.75, help="Delay between requests (default: 0.75)"
    )
