...
class EarningsDipAnalyzer(BaseAnalyzer, FeatureScoringMixin):
        ...
        **kwargs
        ...
        if not drop_info['detected']:
            return self.create_neutral_signal(
                ...
                drop_info.get('reason', 'No earnings dip detected')
            )
        ...
        stab_info = self._check_stabilization(
            prices, highs, lows, volumes, drop_day_idx, opens
        )
        ...
        # =====================================================================
        fund_info = self._check_fundamentals(
            prices, volumes, stability_score
        )
        ...
        # 3. Fundamental Strength (0 – 2.0)
        fund_score = self._score_fundamental_strength(
            stability_score, fund_info
        )
        ...
        overreaction_info = self._score_overreaction(
            dip_pct, prices, volumes, drop_day_idx,
        ...
        # 6. Penalties (-3.0 max)
        penalties_info = self._calculate_penalties(
            prices, lows, drop_day_idx, fund_info
        )
        penalties = penalties_info['total']

        # Total score
        total_raw = drop_score + stab_score + fund_score + overreaction_score + bps_score + penalties
        ...
        signal_text = self._build_signal_text(
            dip_pct, earnings_date, stab_info, stability_score,
            overreaction_info, fund_info
        )
    ...
    def _detect_earnings_drop(
        ...
        if dip_pct > cfg.extreme_dip_pct:
            info['reason'] = f"Dip too extreme ({dip_pct:.1f}% > {cfg.extreme_dip_pct}%) — likely fundamental"
            return info
    ...
    def _check_stabilization(
        ...
        if days_after < self.config.min_stabilization_days:
            info['reason'] = f"Too early — only {days_after} day(s) after drop, need {self.config.min_stabilization_days}"
            return info
    ...
    def _check_fundamentals(
        ...
        if stability_score is not None and stability_score < cfg.min_stability_score:
            ...
            info['reason'] = f"Stability too low ({stability_score:.0f} < {cfg.min_stability_score})"
            return info
    ...
    def _build_signal_text(
        ...
        # Drop info
        if earnings_date:
            date_str = earnings_date.strftime("%b %d") if hasattr(earnings_date, 'strftime') else str(earnings_date)
