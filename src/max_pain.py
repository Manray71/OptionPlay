...
def format_max_pain_report(result: MaxPainResult) -> str:
    ...
    """
    direction = "↓" if result.current_price > result.max_pain else "↑" if result.current_price < result.max_pain else "="
    ...
    lines = [
        ...
        f"───────────────────────────────────────────────────────────",
        f"  Put Wall:   ${result.put_wall:.2f}  ({result.put_wall_oi:,} OI)" if result.put_wall else "  Put Wall:   n/a",
        f"  Call Wall:  ${result.call_wall:.2f}  ({result.call_wall_oi:,} OI)" if result.call_wall else "  Call Wall:  n/a",
        f"",
        f"───────────────────────────────────────────────────────────",
    ...
    if gravity != "neutral":
        hint = "Preis könnte Richtung Max Pain fallen" if gravity == "down" else "Preis könnte Richtung Max Pain steigen"
