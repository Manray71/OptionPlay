...
class AggregatedEarnings:
    ...
    def is_reliable(self) -> bool:
        ...
        return (self.sources_agree >= 2) or (self.confidence >= 60 and self.sources_agree == 1)
...
class EarningsAggregator:
    ...
    def aggregate(self, symbol: str, results: List[EarningsResult]) -> AggregatedEarnings:
        """
        ...
        aggregated = AggregatedEarnings(
            symbol=symbol, all_results=results, total_sources=len(results)
        )

        # Filter successful results with valid dates
        valid_results = [r for r in results if r.success and r.earnings_date]
        ...
        if consensus_date:
            ...
            aggregated.confidence = self._calculate_confidence(agreeing_results, valid_results)
    ...
    def _group_by_date(self, results: List[EarningsResult]) -> Dict[str, List[EarningsResult]]:
        """
    ...
    def _find_consensus(
        self, date_groups: Dict[str, List[EarningsResult]]
    ...
    def _calculate_confidence(
        self, agreeing_results: List[EarningsResult], all_valid_results: List[EarningsResult]
    ) -> int:
        """
    ...
    def _format_discrepancy(self, date_groups: Dict[str, List[EarningsResult]]) -> str:
