...
class RateLimiter:
    ...
    def __init__(
        self, calls_per_minute: int = 100, burst_limit: Optional[int] = None, name: str = "default"
    ) -> None:
        ...
        logger.debug(
            f"RateLimiter '{name}' created: {calls_per_minute}/min, " f"burst={self.burst_limit}"
        )
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "avg_wait_time": (
                round(self._total_wait_time / self._total_waits, 3) if self._total_waits > 0 else 0
            ),
        }
...
class AdaptiveRateLimiter(RateLimiter):
    ...
    def _increase_rate(self) -> None:
        ...
        new_rate = min(self.original_rate, int(self.calls_per_minute * self.recovery_factor))
...
def get_limiter(provider: str, calls_per_minute: int = 100, adaptive: bool = True) -> RateLimiter:
    """
    ...
    if provider not in _limiters:
        if adaptive:
            _limiters[provider] = AdaptiveRateLimiter(
                calls_per_minute=calls_per_minute, name=provider
            )
        else:
