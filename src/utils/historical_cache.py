...
class HistoricalDataCache:
    ...
    def __init__(
        self, ttl_seconds: int = DEFAULT_TTL_SECONDS, max_entries: int = DEFAULT_MAX_ENTRIES
    ) -> None:
        """
    ...
    def get(self, symbol: str, days: int, min_days: Optional[int] = None) -> Optional[Any]:
        """
    ...
    def set(self, symbol: str, data: Any, days: int, ttl_seconds: Optional[int] = None) -> None:
        """
        ...
        with self._lock:
            ...
            self._cache[key] = CacheEntry(
                data=data, created_at=now, expires_at=now + timedelta(seconds=ttl), days=days
            )
...
def get_historical_cache(
    ...
    if _cache_instance is None:
        _cache_instance = HistoricalDataCache(ttl_seconds=ttl_seconds, max_entries=max_entries)
