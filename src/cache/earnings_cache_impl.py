...
class EarningsInfo:
    ...
    def is_safe(
        self, min_days: int = ENTRY_EARNINGS_MIN_DAYS, unknown_is_safe: bool = False
    ) -> bool:
        """
...
class EarningsCache:
    ...
    def __init__(
        self, cache_file: Optional[Path] = None, max_age_hours: int = DEFAULT_CACHE_MAX_AGE_HOURS
    ):
    ...
    def get(self, symbol: str) -> Optional[EarningsInfo]:
        ...
        return EarningsInfo(
            ...
            source=(
                EarningsSource(entry.source)
                if entry.source in [e.value for e in EarningsSource]
                else EarningsSource.UNKNOWN
            ),
...
class EarningsFetcher:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"yfinance failed for {symbol} after {max_retries} attempts: {e}"
                    )
    ...
    def fetch_many(
        self, symbols: List[str], force_refresh: bool = False, progress_callback=None
    ...
    def filter_by_earnings(
        self, symbols: List[str], min_days: int = ENTRY_EARNINGS_MIN_DAYS
