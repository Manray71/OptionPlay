...
class HistoricalCache:
    ...
    def get(self, symbol: str, days: int, accept_more_days: bool = True) -> CacheLookupResult:
        """
        ...
        with self._lock:
            ...
            self._misses += 1
            return CacheLookupResult(
                status=CacheStatus.MISS, message=f"No cache entry for {symbol}:{days}"
            )
...
def get_historical_cache(
    ...
    if _cache_instance is None:
        _cache_instance = HistoricalCache(ttl_seconds=ttl_seconds, max_entries=max_entries)
