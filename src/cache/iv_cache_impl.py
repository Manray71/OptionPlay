...
class IVData:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "iv_percentile": (
                round(self.iv_percentile, 1) if self.iv_percentile is not None else None
            ),
...
class IVCache:
    ...
    def __init__(
        self, cache_file: Optional[Path] = None, max_age_days: int = DEFAULT_CACHE_MAX_AGE_DAYS
    ):
    ...
    def get_iv_data(self, symbol: str, current_iv: Optional[float] = None) -> Optional[IVData]:
        ...
        return IVData(
            ...
            data_points=entry.data_points,
            source=(
                IVSource(entry.source)
                if entry.source in [e.value for e in IVSource]
                else IVSource.UNKNOWN
            ),
            updated_at=entry.updated,
        )

    def update_history(
        self, symbol: str, iv_history: List[float], source: IVSource = IVSource.UNKNOWN
    ) -> None:
        """
    ...
    def add_iv_point(
        self, symbol: str, iv_value: float, source: IVSource = IVSource.UNKNOWN
    ) -> None:
        """
    ...
    def add_iv_points_batch(self, entries: List[Tuple[str, float, IVSource]]) -> int:
        """
...
class IVFetcher:
    ...
    def get_iv_rank_many(self, symbols_with_iv: List[Tuple[str, float]]) -> Dict[str, IVData]:
        """
        ...
        """
        return {
            symbol: self.get_iv_rank(symbol, current_iv) for symbol, current_iv in symbols_with_iv
        }

    def extract_atm_iv_from_chain(
        self, options_chain: List[Dict], underlying_price: float
    ) -> Optional[float]:
        """
    ...
    def update_from_chain(
        self, symbol: str, options_chain: List[Dict], underlying_price: float
    ) -> Optional[float]:
        """
...
class HistoricalIVFetcher:
    ...
    def fetch_iv_history(
        self, symbol: str, days: int = 252, use_vix_adjustment: bool = True
    ) -> List[float]:
        """
    ...
    def update_cache_for_symbol(self, symbol: str, days: int = 252, force: bool = False) -> bool:
        """
    ...
    def update_cache_for_symbols(
        self, symbols: List[str], days: int = 252, force: bool = False, delay_seconds: float = 0.5
    ) -> Dict[str, bool]:
        """
