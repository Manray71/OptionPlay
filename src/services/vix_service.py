...
class VIXService(BaseService):
    ...
    async def get_vix(self, force_refresh: bool = False) -> ServiceResult[float]:
        ...
        if not force_refresh and self._context._vix_cache and self._context._vix_updated:
            ...
            if age < vix_cache_seconds:
                return ServiceResult.ok(data=self._context._vix_cache, source="cache", cached=True)
        ...
        if vix:
            ...
            return ServiceResult.ok(data=vix, source=source, cached=False, duration_ms=duration_ms)
        ...
        return ServiceResult.fail(
            error="Could not fetch VIX from any source", duration_ms=duration_ms
        )
    ...
    async def get_vix_concurrent(self) -> ServiceResult[float]:
        ...
        done, pending = await asyncio.wait(tasks, return_when=asyncio.FIRST_COMPLETED, timeout=10.0)
        ...
        for task in done:
            try:
                ...
                if result:
                    ...
                    return ServiceResult.ok(
                        data=vix, source=source, cached=False, duration_ms=duration_ms
                    )
            except Exception as e:
    ...
    async def get_strategy_recommendation(self) -> ServiceResult[StrategyRecommendation]:
        ...
        return ServiceResult.ok(
            data=recommendation, source=vix_result.source, cached=vix_result.cached
        )
