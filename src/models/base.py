...
try:
    ...
    from src.utils.validation import (  # noqa: F401 — fallback for non-package imports
        ValidationError,
    )
...
class TradeSignal:
    ...
    reliability_ci: Optional[tuple[float, float]] = None  # Confidence Interval (low, high)
    ...
    def __post_init__(self) -> None:
        ...
        if self.current_price is not None:
            if not isinstance(self.current_price, (int, float)):  # type: ignore[unreachable]
                errors.append(f"current_price must be numeric, got {type(self.current_price)}")  # type: ignore[unreachable]
            elif self.current_price <= 0:
                errors.append(f"current_price must be positive, got {self.current_price}")

        if self.entry_price is not None:
            if not isinstance(self.entry_price, (int, float)):  # type: ignore[unreachable]
                errors.append(f"entry_price must be numeric, got {type(self.entry_price)}")  # type: ignore[unreachable]
            elif self.entry_price <= 0:
                errors.append(f"entry_price must be positive, got {self.entry_price}")

        if self.stop_loss is not None:
            if not isinstance(self.stop_loss, (int, float)):  # type: ignore[unreachable]
                errors.append(f"stop_loss must be numeric, got {type(self.stop_loss)}")  # type: ignore[unreachable]
            elif self.stop_loss <= 0:
                errors.append(f"stop_loss must be positive, got {self.stop_loss}")

        if self.target_price is not None:
            if not isinstance(self.target_price, (int, float)):  # type: ignore[unreachable]
                errors.append(f"target_price must be numeric, got {type(self.target_price)}")  # type: ignore[unreachable]
        ...
        if self.reliability_win_rate is not None:
            if not isinstance(self.reliability_win_rate, (int, float)):  # type: ignore[unreachable]
                errors.append(f"reliability_win_rate must be numeric")  # type: ignore[unreachable]
        ...
        if self.reliability_ci is not None:
            if not isinstance(self.reliability_ci, (tuple, list)) or len(self.reliability_ci) != 2:  # type: ignore[unreachable]
                errors.append("reliability_ci must be a tuple of (low, high)")  # type: ignore[unreachable]
            else:
