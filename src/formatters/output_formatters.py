    ...
    def format(
        ...
        if result.signals:
            ...
            if recommendation:
                ...
                b.numbered(
                    f"Options chain: `get_options_chain <SYMBOL>` (DTE {SPREAD_DTE_MIN}-{SPREAD_DTE_MAX}, Delta {recommendation.delta_target})"
                )
                b.numbered(
                    f"Spread width: {'Dynamic (delta-based)' if recommendation.spread_width is None else f'${recommendation.spread_width:.2f}'}"
                )
        else:
...
class LegacyScanResultFormatter(BaseFormatter):
    ...
    def format(
        self, result: ScanResultProtocol, min_score: float = 5.0, max_results: int = 10
    ) -> str:
        b = self._builder()
...
class QuoteFormatter(BaseFormatter):
    ...
    def format(self, symbol: str, quote: Optional[QuoteProtocol]) -> str:
        b = self._builder()
    ...
    def format(
        ...
        for expiry in sorted(by_expiry.keys()):
            ...
            for opt in sorted_opts:
                ...
                atm_marker = (
                    " ◄" if underlying_price and abs(opt.strike - underlying_price) < 2.5 else ""
                )
...
class StrategyRecommendationFormatter(BaseFormatter):
    ...
    def format(self, recommendation: StrategyRecommendationProtocol, vix: Optional[float]) -> str:
        b = self._builder()
...
class HealthCheckFormatter(BaseFormatter):
    ...
    def format(self, data: HealthCheckData) -> str:
        ...
        state_icon = (
            "✅ Closed"
            if cb["state"] == "closed"
            else "❌ OPEN" if cb["state"] == "open" else "⚠️ Half-Open"
        )
        ...
        if data.local_db_enabled and data.local_db_stats:
            ...
            b.kv_line(
                "Date Range", f"{stats.get('min_date', 'N/A')} to {stats.get('max_date', 'N/A')}"
            )
...
class HistoricalDataFormatter(BaseFormatter):
    ...
    def format(
        self, symbol: str, bars: List[Any], days_shown: int = 10  # List of HistoricalBar
    ) -> str:
        b = self._builder()
    ...
    def format(
        ...
        if historical:
            ...
            if pullback_signal:
                b.h2("Pullback Score")
                b.kv_line(
                    "Score", f"{pullback_signal.score:.1f}/10 (Min: {recommendation.min_score})"
                )
        ...
        if (
            earnings
            and earnings.days_to_earnings
            and earnings.days_to_earnings < recommendation.earnings_buffer_days
        ):
            warnings.append(
                f"Earnings in {earnings.days_to_earnings} days (Min: {recommendation.earnings_buffer_days})"
            )
