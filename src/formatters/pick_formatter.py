...
try:
    ...
    from utils.markdown_builder import (  # type: ignore[no-redef]  # fallback for non-package execution
        MarkdownBuilder,
        truncate,
    )
...
def format_single_pick_v2(b: MarkdownBuilder, pick: Any) -> None:
    ...
    Args:
        ...
        pick: DailyPick
    """
    strategy_str = _STRATEGY_DISPLAY.get(pick.strategy, pick.strategy.replace("_", " ").title())
