...
class SignalAggregator:
    ...
    def aggregate(
        self, signals: List[TradeSignal], min_agreement: int = 1
    ...
    def filter_by_strategy(self, signals: List[TradeSignal], strategy: str) -> List[TradeSignal]:
        """Filtert Signale nach Strategie"""
        return [s for s in signals if s.strategy == strategy]

    def get_multi_strategy_hits(
        self, signals: List[TradeSignal], min_strategies: int = 2
    ) -> List[AggregatedSignal]:
        """
        ...
        return [
            agg
            for agg in self.aggregate(signals, min_agreement=min_strategies)
